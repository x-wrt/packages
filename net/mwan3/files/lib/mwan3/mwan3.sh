...
if ! $IP6 route >/dev/null 2>&1; then
	IP6=""
fi

# return true(=0) if has any mwan3 ipv6 interface enabled
...
mwan3_need_ipv6()
{
	...
	while uci get mwan3.@interface[$idx] >/dev/null 2>&1 ; do
		if [ "$(uci get mwan3.@interface[$idx].family)" = "ipv6" ] && [ "x$(uci get mwan3.@interface[$idx].enabled)" = "x1" ]; then
			return 0
		fi
		idx=$((idx+1))
	done
...
mwan3_set_custom_ipset()
{
	...
	test -n "$IP6" && mwan3_need_ipv6 || return 0
...
mwan3_set_general_rules()
{
	...
	for IP in "$IP4" "$IP6"; do
		...
		([ "x$IP" = x"$IP6" ] && ! mwan3_need_ipv6 || ! test -n "$IP") && continue
...
mwan3_set_user_iptables_rule()
{
	...
	if [ -n "$use_policy" ]; then
		...
		elif [ "$family" == "ipv6" ] && test -n "$IPT6"; then
